...
static void CalcTBN(const mesh_vertex_t *vertices, uint32_t vcount,
	const mesh_index_t *indices, uint32_t icount, VertexTB *tb)
{
	ASSERT(vertices);
	ASSERT(indices);
	ASSERT(tb);
...
bool Mesh::load(const char *name, bool calcTBN, float scale)
{
	ASSERT(name);
	ASSERT(m_vao == 0);
